...
<div className="modal fade" id="exampleModal" tabIndex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div className="modal-dialog">
    <div className="modal-content">
      <div className="modal-header">
        <h1 style={{color:'green'}} className="modal-title fs-5" id="exampleModalLabel">Edit Your Note</h1>
 ...
 style={{backgroundColor:'pink',Color:'red'}} onClick={()=>{submitUpdate(updateProduct._id)}} type="submit" >Add Product</button>
    </div>
          </div>
      </div>
    ...
    <div>
      {error && <p>{error}</p>}
      <h2 style={{color:'Purple',backgroundColor:'skyblue'}} className="text-center">Product List</h2>
      <ul className="list-group mt-4">
        {products.map((product) => (
            <li style={{color:'green'}}key={product._id} className="list-group-item list-group-item-success d-flex justify-content-between">
              {/* onClick={()=>{updateClick(note)}} */}
              <div>{product.name} - {product.price}</div>
              <div>
              <i style={{color:'purple'}}className="fa fa-pencil mb-2" type="button" onClick={()=>{updateHandler(product._id,product.name,product.description,product.price,product.category)}}data-bs-toggle="modal" data-bs-target="#exampleModal" aria-hidden="true"></i>
                <i style={{color:'brown'}} className="fa fa-trash mx-3 mb-2" onClick={()=>{deleteHandler(product._id)}} aria-hidden="true"></i>
              </div>
          </li>      
        ))}
      </ul>
